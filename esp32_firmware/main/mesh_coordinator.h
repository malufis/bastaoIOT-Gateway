 ...
 * @brief Cabecalho do modulo coordenador e provisionador da rede local BLE Mesh.
 * @details Este modulo expoe a interface publica para inicializar a rede de malha BLE,
 ...
 * 
 * @pre O driver de NVS do ESP32 ja deve ter sido previamente inicializado.
 * @post O ESP32 inicia a escuta por novos nos nao-provisionados na rede (como a Tela K10).
 * 
 * @return esp_err_t ESP_OK em caso de sucesso, ou erro de driver especifico.
 */
esp_err_t mesh_coordinator_init(void);

/**
 * @brief Envia um payload criptografado via BLE Mesh para os nos cadastrados (ex: Tela K10).
 ...
 * 
 * @pre A pilha BLE Mesh deve estar operacional (mesh_coordinator_init concluido).
 * @post A transmissao e agendada no loop de execucao do BLE.
 * 
